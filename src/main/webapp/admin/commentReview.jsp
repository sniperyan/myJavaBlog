...
<script type="text/javascript">
	...
	function commentReview(state){
		...
		$.messager.confirm("系统提示","您确定要审核这<font color=red>"+selectedRows.length+"</font>条评论吗？",function(r){
			if(r){
				$.post("${webctx}/admin/comment/review.do",{ids:ids,state:state},function(result){
					if(result.success){
	...
	function formatBlogTitle(val,row){
		if(val==null){
			...
			return "<a target='_blank' href='${webctx}/blog/articles/"+val.id+".html'>"+val.title+"</a>";			
		}
	}
...
<table id="dg" title="评论审核管理" class="easyui-datagrid"
   ...
   url="${webctx}/admin/comment/list.do?state=0" fit="true" toolbar="#tb">
   <thead>
   	<tr>
